...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchProfile() {
      ...
      if (user) {
        ...
        const { greeting: newGreeting } = getGreeting(
          profileData?.name || user.email?.split("@")[0] || "there"
        );
  ...
  // Derive greeting from profile instead of using effect
  const userName = profile?.name || "there";
  const { greeting: derivedGreeting } = getGreeting(userName);
  const finalGreeting = greeting || derivedGreeting;
